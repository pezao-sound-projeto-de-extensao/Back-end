...
class EncomendaServiceTest {
    ...
    private OrcamentoItem orcamentoItem(Item item, String descricao, int quantidade) {
    ...
    private Encomenda encomenda(Integer id, Item item, StatusEncomenda status, int quantidade) {
        Encomenda encomenda = new Encomenda();
        encomenda.setId(id);
        encomenda.setOrcamento(orcamento);
        encomenda.setOrcamentoItem(orcamentoItem(item, "Módulo Amplificador 400W", quantidade));
    ...
    class GeracaoTest {
        ...
        void deveGerarUmaPorItem() {
            orcamento.adicionarItem(orcamentoItem(amplificador, "Módulo Amplificador 400W", 2));
            orcamento.adicionarItem(orcamentoItem(null, "Kit de fiação 4mm", 3));
            ...
            assertTrue(geradas.stream().allMatch(e -> e.getStatus() == StatusEncomenda.PENDENTE));
    ...
    class RecebimentoTest {
        ...
        void deveReceber() {
            Encomenda pendente = encomenda(1, amplificador, StatusEncomenda.PENDENTE, 2);

            when(repository.findById(1)).thenReturn(Optional.of(pendente));
            when(itemRepository.findByIdParaMovimentacao(10)).thenReturn(Optional.of(amplificador));
            when(estoqueService.aplicarMovimentacao(amplificador, TipoMovimentacao.ENTRADA, 2))
                    .thenAnswer(inv -> {
        ...
        void deveRecusarProdutoNaoCadastradoSemItemId() {
            Encomenda pendente = encomenda(1, null, StatusEncomenda.PENDENTE, 3);
        ...
        void deveRecusarProdutoInexistente() {
            ...
            assertThrows(EntityNotFoundException.class, () -> service.receber(1, 99));
        }
        ...
        void naoDeveReceberDuasVezes() {
            when(repository.findById(1))
                    .thenReturn(Optional.of(encomenda(1, amplificador, StatusEncomenda.RECEBIDA, 2)));

            assertThrows(IllegalArgumentException.class, () -> service.receber(1, null));
    ...
    class ConclusaoTest {
        ...
        void deveConcluir() {
            Encomenda recebida = encomenda(1, amplificador, StatusEncomenda.RECEBIDA, 2);

            when(repository.findById(1)).thenReturn(Optional.of(recebida));
            when(itemRepository.findByIdParaMovimentacao(10)).thenReturn(Optional.of(amplificador));
            when(estoqueService.aplicarMovimentacao(amplificador, TipoMovimentacao.SAIDA, 2))
                    .thenAnswer(inv -> {
                        amplificador.setQuantidadeAtual(6);
                        return 8;
                    });
            when(repository.findByOrcamento_Id(5)).thenReturn(List.of(recebida));
            mockarSalvamento();
        ...
        void naoDeveConcluirPendente() {
            when(repository.findById(1))
                    .thenReturn(Optional.of(encomenda(1, amplificador, StatusEncomenda.PENDENTE, 2)));

            assertThrows(IllegalArgumentException.class, () -> service.concluir(1));
        ...
        void deveConcluirOrcamento() {
            Encomenda recebida = encomenda(1, amplificador, StatusEncomenda.RECEBIDA, 2);
            Encomenda jaConcluida = encomenda(2, amplificador, StatusEncomenda.CONCLUIDA, 1);

            when(repository.findById(1)).thenReturn(Optional.of(recebida));
            when(itemRepository.findByIdParaMovimentacao(10)).thenReturn(Optional.of(amplificador));
            when(repository.findByOrcamento_Id(5)).thenReturn(List.of(recebida, jaConcluida));
            mockarSalvamento();
        ...
        void naoDeveConcluirOrcamentoComPendencias() {
            Encomenda recebida = encomenda(1, amplificador, StatusEncomenda.RECEBIDA, 2);
            Encomenda pendente = encomenda(2, amplificador, StatusEncomenda.PENDENTE, 1);

            when(repository.findById(1)).thenReturn(Optional.of(recebida));
            when(itemRepository.findByIdParaMovimentacao(10)).thenReturn(Optional.of(amplificador));
            when(repository.findByOrcamento_Id(5)).thenReturn(List.of(recebida, pendente));
            mockarSalvamento();
        ...
        void naoDeveConcluirSemEstoque() {
            ...
            doThrow(new EstoqueInsuficienteException(1, 2))
                    .when(estoqueService)
                    .aplicarMovimentacao(amplificador, TipoMovimentacao.SAIDA, 2);

            assertThrows(EstoqueInsuficienteException.class, () -> service.concluir(1));
    ...
    class ListagemTest {
        ...
        void deveRepassarFiltros() {
            when(repository.findAllFiltered("joão", StatusEncomenda.PENDENTE, pageable))
                    .thenReturn(new PageImpl<>(List.of(encomenda(1, amplificador, StatusEncomenda.PENDENTE, 2))));

            service.listar("joão", "pendente", pageable);

            verify(repository).findAllFiltered("joão", StatusEncomenda.PENDENTE, pageable);
        }
        ...
        void deveMontarKpis() {
            ...
            when(repository.countByStatusAndConcluidaEmBetween(
                    eq(StatusEncomenda.CONCLUIDA), any(LocalDateTime.class), any(LocalDateTime.class)))
                    .thenReturn(7L);
